...
class Pinns:
    def __init__(self, config):
        ...
        self.domain_extrema = torch.tensor([[0, 0.001],  # Time dimension
                                            [0, 1],  # x dimension
                                            [0, 1]])  # y dimension
        ...
        self.training_set_sb, self.training_set_tb, self.training_set_etb, self.training_set_int = self.assemble_datasets()
    ...
    def add_temporal_boundary_points(self):
        ...
        x_01 = torch.linspace(0, 1, 51)
        y_01 = torch.cat([torch.linspace(0, 0.48, 25), torch.linspace(0.52, 1, 25)])
        ...
        y_02 = torch.linspace(0.49, 0.51, 3)
        ...
        input_tb_space = torch.cat([input_tb_0, input_tb_1], 0)
        input_tb = torch.cat((input_tb_time, input_tb_space), dim=1)

        return input_tb

    def add_end_temporal_boundary_points(self):
        # 从时间坐标中获取结束时间
        t_end = torch.tensor([self.domain_extrema[0, 1]])
        input_etb_time = torch.tile(t_end[:, None], [self.n_tb, 1])
        input_etb_space = self.strueng.generate_structure_grid()
        input_etb = torch.cat((input_etb_time, input_etb_space), dim=1)
    ...
    def add_spatial_boundary_points(self):
        ...
        input_sb_B = torch.clone(input_sb)
        input_sb_B[:, 2] = y0   #下边界(bottom boundary)
        input_sb_U = torch.clone(input_sb)
        input_sb_U[:, 2] = yL   #上边界(upper boundary)
        input_sb = torch.cat([input_sb_U, input_sb_B], 0)

        return input_sb

    def add_interior_points(self):
        t = torch.linspace(self.domain_extrema[0, 0], self.domain_extrema[0, 1], 3)
        input_int_time = torch.tile(t[:, None], [self.n_int, 1])

        x_01 = torch.linspace(0, 1, 51)
        y_01 = torch.cat([torch.linspace(0, 0.48, 25), torch.linspace(0.52, 1, 25)])
        grid_x01, grid_y01 = torch.meshgrid(x_01, y_01)
        input_int_01 = torch.stack((grid_x01.reshape(-1), grid_y01.reshape(-1)), dim=1)
        x_02 = torch.linspace(0.51, 1, 50)
        y_02 = torch.linspace(0.49, 0.51, 3)
        grid_x02, grid_y02 = torch.meshgrid(x_02, y_02)
        input_int_02 = torch.stack((grid_x02.reshape(-1), grid_y02.reshape(-1)), dim=1)
        input_int_0 = torch.cat([input_int_01, input_int_02], 0)
        # 在空间坐标上均匀取样裂纹部分
        x_1 = torch.linspace(0, 0.5, 51)
        y_1 = torch.full((51, 1), 0.5)
        input_int_1 = torch.cat([x_1.unsqueeze(1), y_1], dim=1)
        input_int_space = torch.cat([input_int_0, input_int_1], 0)
        input_int_space = torch.tile(input_int_space, [3, 1])
        # 将时间坐标与空间坐标连接在一起
        input_int = torch.cat((input_int_time, input_int_space), dim=1)
    ...
    def assemble_datasets(self):
        ...
        training_set_sb = DataLoader(torch.utils.data.TensorDataset(input_sb),
                                     batch_size=self.space_dimensions * self.n_sb, shuffle=False)
        ...
        training_set_int = DataLoader(torch.utils.data.TensorDataset(input_int),
                                      batch_size=3 * self.n_int, shuffle=False)

        return training_set_sb, training_set_tb, training_set_etb, training_set_int

    def compute_ic0_residual(self, input_tb):  #未裂部分初始条件
    ...
    def compute_ic1_residual(self, input_tb):  #裂纹部分初始条件
        input_tb.requires_grad = True
        u = self.approximate_solution(input_tb)
        u1 = u[:, 0].reshape(-1, 1)
        u2 = u[:, 1].reshape(-1, 1)
    ...
    def compute_bcU_residual(self, input_sb):  #上边界(upper boundary)
        input_sb.requires_grad = True
        u = self.approximate_solution(input_sb)
        u2 = u[:, 1].reshape(-1, 1)
        residual_U_2 = u2 - input_sb[:, 0]

        return residual_U_2.reshape(-1, )

    def compute_bcB_residual(self, input_sb):  #下边界(bottom boundary)
        input_sb.requires_grad = True
        u = self.approximate_solution(input_sb)
        u1 = u[:, 0].reshape(-1, 1)
        u2 = u[:, 1].reshape(-1, 1)
    ...
    def compute_loss(self, inp_train_sb,  inp_train_tb,  inp_train_int, verbose=True):
        ...
        r_sbB_u1, r_sbB_u2 = self.compute_bcB_residual(inp_train_sb[self.n_sb:, :])
        r_tb_u1_0, r_tb_u2_0, r_tb_c_0, = self.compute_ic0_residual(inp_train_tb[0:2700, :])
        r_tb_u1_1, r_tb_u2_1, r_tb_c_1, = self.compute_ic1_residual(inp_train_tb[2700:, :])
    ...
    def fit(self, num_epochs, max_iter, lr, verbose=True):
        ...
        with torch.no_grad():
            ...
            output_filename = f"output_1e-3.csv"
            with open(output_filename, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerows(solution.cpu().numpy())
        #Plot2D.Contour2D(nodecoords=np.array(list(self.training_set_etb)[0][0]), sol=u_end.cpu().numpy(), savefig=True, figname='result')
    ...
    def plotting(self):
        ...
        for i, t_val in enumerate(t):
            if i % 1 == 0:
                ...
                im3 = axs[2].imshow(c_reshaped, cmap='jet', origin='lower',
                                    extent=[x.min(), x.max(), y.min(), y.max()])
                axs[2].set_xlabel("x")
                axs[2].set_ylabel("y")
...
class Pinns2:
    ...
    def add_spatial_boundary_points(self):
        ...
        input_sb_B = torch.cat((input_sb_space_B, input_sb_time), dim=1)
        input_sb_space_U = torch.clone(input_sb_space)
        input_sb_space_U[:, 1] = yL
        input_sb_space_U = torch.tile(input_sb_space_U, [2, 1])
        input_sb_U = torch.cat((input_sb_space_U, input_sb_time), dim=1)
        input_sb = torch.cat([input_sb_U, input_sb_B], 0)
    ...
    def compute_ic_residual(self, input_itb):
        ...
        residual_c = c - c_previous

        return residual_u1.reshape(-1, ), residual_u2.reshape(-1, ), residual_c.reshape(-1, )

    def compute_bcU_residual(self, input_sb):
        input_sb.requires_grad = True
        u = self.approximate_solution(input_sb)
        u2 = u[:, 1].reshape(-1, 1)
        residual_U_2 = u2 - 0.01 * input_sb[:, 2]
    ...
    def fit(self, num_epochs, max_iter, lr, verbose=True):
        ...
        if self.optimizer_name == "lbfgs":
            ...
            for j, (batch_sb, batch_itb, batch_int) in enumerate(zip(
                    self.training_set_sb, self.training_set_itb, self.training_set_int)):
