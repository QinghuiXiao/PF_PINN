...
class Pinns:
    ...
    def add_temporal_boundary_points(self):
        ...
        x_01 = torch.linspace(0, 0.5, 11)
        y_01 = torch.cat([torch.linspace(0, 0.49, 10), torch.linspace(0.51, 1, 10)])
        ...
        # 在空间坐标上均匀取样裂纹部分
        x_1 = torch.linspace(0, 0.5, 11)
        y_1 = torch.full((11, 1), 0.5)
    ...
    def compute_bcU_residual(self, input_sb):
        ...
        u2 = u[:, 1].reshape(-1, 1)
        residual_U_2 = u2 - 0.00001 * input_sb[:, 0]
...
class Pinns2:
    ...
    def add_temporal_boundary_points(self):
        ...
        input_tb_0 = torch.stack((grid_x0.reshape(-1), grid_y0.reshape(-1)), dim=1)
        #x_02 = torch.linspace(0.51, 1, 10)
        #y_02 = torch.linspace(0, 1, 21)
        #grid_x02, grid_y02 = torch.meshgrid(x_02, y_02)
        #input_tb_02 = torch.stack((grid_x02.reshape(-1), grid_y02.reshape(-1)), dim=1)
    ...
    def add_spatial_boundary_points(self):
        ...
        input_sb_D = torch.cat((input_sb_space_D, input_sb_time), dim=1)
    ...
    def add_interior_points(self):
        ...
        input_int_time = torch.tile(t_end[:, None], [self.n_tb, 1])
        ...
        input_int = torch.cat([input_int_0, input_int_1], 0)
        input_int = torch.cat((input_int, input_int_time), dim=1)
        return input_int
    ...
    def compute_bcU_residual(self, input_sb):
        ...
        u = self.approximate_solution(input_sb)
        u2 = u[:, 1].reshape(-1, 1)
        residual_U_2 = u2 - 0.0001 * input_sb[:, 2]
        return  residual_U_2.reshape(-1, )
    ...
    def compute_loss(self, inp_train_sb,  inp_train_tb,  inp_train_int, verbose=True):
        ...
        r_sbD_u1, r_sbD_u2 = self.compute_bcD_residual(inp_train_sb[2*self.n_sb:, :])
        r_tb_u1, r_tb_u2, r_tb_c, = self.compute_ic_residual(inp_train_tb)
    ...
    def fit(self, num_epochs, max_iter, lr, verbose=True):
        ...
        with torch.no_grad():
            ...
            solution = torch.cat((list(self.training_set_int)[0][0], u_end), 1)
